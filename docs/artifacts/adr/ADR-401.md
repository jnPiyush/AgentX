...
| Commit-time hook is bash but delegates the gate decision to PowerShell | `#!/usr/bin/env bash` plus `agentx loop gate` | `.github/hooks/pre-commit` L1, 2026-05-29 |
...
- `.github/hooks/pre-commit` -- bash commit-time hook delegating to `agentx loop gate`
