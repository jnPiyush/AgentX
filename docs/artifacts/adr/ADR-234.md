...
- Existing package forms already exist at the pack level in `packs/frontier-core/manifest.json` and `packs/frontier-copilot-cli/manifest.json`
...
We will publish AgentX plugins as independent packages on top of `frontier-core`, using a registry index plus versioned release artifacts as the first production publication model.

**Key architectural choices:**
- `frontier-core` remains the host/runtime contract for plugin execution and lifecycle
...
- `packs/frontier-core/manifest.json`
