...
VS Code has shipped a new agent-first surface called the **Agents Window** (Preview, May 2026) that is positioned as the strategic home for orchestrating long-running, multi-project AI agent work. Today, AgentX (`vscode-extension` v8.4.62) ships as a `chatParticipants`-based extension whose primary surfaces are the editor sidebar (tree views, status bar, commands) and a chat participant in the main VS Code chat view. AgentX must decide how its 26 agents, 134 skills, workflow gates, and quality-loop CLI show up inside the new Agents Window without forcing users to abandon the editor-window experience.
...
| 26 agents in `.github/agents/*.agent.md` | Frontmatter already matches the VS Code `.agent.md` schema | repo structure |
| 134 skills under `.github/skills/**/SKILL.md` | Already in the format the Customizations panel expects under "Skills" | `Skills.md` |
...
flowchart TB
  ...
  subgraph AGENTX["AgentX delivery"]
    ...
    SKILLS[".github/skills/**/SKILL.md<br/>(134 skills)"]
