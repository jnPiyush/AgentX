...
.EXAMPLE
  pwsh scripts/autoresearch.ps1 -Action start -Goal "shrink build time" -Metric "npm run build:time" -Direction lower -Tag build-time

.EXAMPLE
  pwsh scripts/autoresearch.ps1 -Action attempt -Note "drop unused webpack plugin"

.EXAMPLE
  pwsh scripts/autoresearch.ps1 -Action end
#>
...
$StateDir  = Join-Path $RootDir '.agentx/state/autoresearch'
...
switch ($Action) {
    'start' {
        ...
        Write-Host "[autoresearch] Capturing baseline..." -ForegroundColor Cyan
        ...
        Write-Host ("[autoresearch] Started '$Tag' on branch $branch") -ForegroundColor Green
    ...
    'attempt' {
        ...
        Write-Host "[autoresearch] Attempt $attemptNum running metric..." -ForegroundColor Cyan
        ...
        if ($isImprovement) {
            & git add -A | Out-Null
            $msg = "autoresearch($($state.tag)): attempt $attemptNum kept ($baseline -> $value)"
            if ($Note) { $msg += " - $Note" }
            & git commit -m $msg | Out-Null
            $state.kept = [int]$state.kept + 1
            $state.currentBest = $value
            Write-Host ("[autoresearch] KEEP. {0} -> {1} (delta {2:N4})" -f $baseline, $value, $delta) -ForegroundColor Green
        } else {
            & git checkout -- . 2>&1 | Out-Null
            & git clean -fd 2>&1 | Out-Null
            $state.reverted = [int]$state.reverted + 1
            Write-Host ("[autoresearch] REVERT. {0} did not beat {1} (delta {2:N4})" -f $value, $baseline, $delta) -ForegroundColor Yellow
        }
    ...
    'status' {
        $state = Get-State
        if (-not $state -or -not $state.active) { Write-Host "[autoresearch] No active experiment."; return }
        Write-Host "[autoresearch] Active experiment" -ForegroundColor Cyan
    ...
    'end' {
        ...
        Save-State $state
        Write-Host "[autoresearch] Closed experiment '$($state.tag)'." -ForegroundColor Green
