...
'@ 'development'
    ...
    $expectedSkillCount = @(Get-ChildItem (Join-Path $root '.github/skills') -Recurse -Filter SKILL.md -File).Count
    $allJson = & $scorer -All -Json 2>$null | Out-String
    $all = $allJson | ConvertFrom-Json -Depth 20
    Assert-True ($all.total -eq $expectedSkillCount) 'all-skills mode scores the complete canonical skill inventory'
    ...
    Assert-True ($stocktakeResult.total -eq $expectedSkillCount) 'stocktake consumes all rubric results'
