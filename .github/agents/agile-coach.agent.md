...
Story output follows As a/I want/So that format, satisfies all INVEST dimensions, includes 3-7 Given/When/Then acceptance criteria, covers happy path/error/boundary behavior, and is confirmed or marked pending confirmation by the user.

## Delivery Report (MANDATORY)

Before handoff, report: story format status; INVEST score; acceptance-criteria count; GWT compliance; happy path/error/boundary coverage; user confirmation state; and Frontier quality-loop state.
