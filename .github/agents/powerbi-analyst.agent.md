...
Report/model renders or is specified correctly; DAX measures are validated; semantic model and relationships are documented; RLS/performance constraints are handled when required; and no embedded credentials remain.

## Delivery Report (MANDATORY)

Before handoff, report: render status; DAX measures validated; semantic model documentation; embedded credential findings; RLS status; performance notes; and Frontier quality-loop state.
