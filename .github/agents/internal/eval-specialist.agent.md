...
Evaluation framework uses structured rubrics; judge is validated against known-answer data when available; at least two models/providers are compared when required; quality gates and result documentation are complete.

## Delivery Report (MANDATORY)

Before handoff, report: rubric status; judge agreement; model count/provider diversity; primary and fallback scores; result path; and Frontier quality-loop state.
