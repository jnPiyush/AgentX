...
def scaffold(name: str, component: str, vector_store: str) -> None:
    ...
    if component in ("rag", "all"):
        ...
        create_file(
            root / "src" / module / "rag" / "config.py", _rag_config(name, vector_store)
        )
        create_file(
            root / "src" / module / "rag" / "ingestion.py",
            _rag_ingestion(name, vector_store),
        )
        create_file(
            root / "src" / module / "rag" / "retrieval.py", _rag_retrieval(name)
        )
    ...
    if component in ("memory", "all"):
        ...
        create_file(
            root / "src" / module / "memory" / "manager.py", _memory_manager(name)
        )
