# Power Platform Solution Structure Detail

> Read this when the [solution-anatomy](../SKILL.md) root tells you to build the
> unpacked tree, populate `Solution.xml`, name components, or troubleshoot a
> pack/import failure. Content below is relocated verbatim from the original
> root so the technical detail remains available without keeping the root over
> budget.
...
<solution-root>/
  src/
    ...
    connectionreferences.json # package-level connection bindings for import
