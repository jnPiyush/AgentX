...
def build_path_item(method: str, path: str, resource: str) -> dict:
    ...
    if method == "get" and not path_params:
        ...
        operation["requestBody"] = {
            ...
            "content": {
                "application/json": {
                    "schema": {"$ref": f"#/components/schemas/{resource}Create"}
                }
            },
        }
        ...
        operation["requestBody"] = {
            ...
            "content": {
                "application/json": {
                    "schema": {"$ref": f"#/components/schemas/{resource}Update"}
                }
            },
        }
        operation["responses"]["200"] = {
...
def generate_spec(
    ...
    spec: dict = {
        ...
        "servers": [
            ...
            {
                "url": base_url.replace("localhost", "api.example.com"),
    ...
    for endpoint_str in endpoints:
        ...
        if resource not in resources_seen:
            ...
            spec["tags"].append(
                {
                    "name": resource + "s",
                    "description": f"Operations on {resource.lower()}s",
                }
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scaffold an OpenAPI 3.1 specification"
    )
