# DOCX Operations Detail

> Read this when the [docx](../SKILL.md) root tells you to choose a library,
> perform a specific `.docx` operation, or inspect a package-level failure.
> Content below is relocated verbatim from the original root so detailed recipes
> remain available without keeping the root over budget.
...
| Legacy `.doc` (binary) | LibreOffice or another `.doc`-capable converter | Convert to `.docx` first; `python-docx` does NOT support `.doc` |
...
On Windows or other shells without `unzip` and `head`, inspect the package with Python instead:

```python
from zipfile import ZipFile

with ZipFile("file.docx") as z:
    print(z.namelist())
    print(z.read("word/document.xml").decode("utf-8")[:1000])
```
