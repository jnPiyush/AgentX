# Frontier Power Platform FDE Pack

Pack that lets Frontier generate **unpacked Microsoft Power Platform solution source trees** from a PRD-LOWCODE. The maker then runs `pac solution pack` and `pac solution import` to deploy into a Dataverse environment. Coverage spans Tier-1 (data + flows), Tier-2 (apps, RPA, portals, PCF, plugins, security, environment variables), and Tier-3 (Copilot Studio agents).
...
2. Invoke the Frontier Power Platform FDE and point it at the PRD.
