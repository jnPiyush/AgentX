...
<#
.SYNOPSIS
 Install Frontier Copilot CLI Plugin v9.3.0 into a workspace.
...
.DESCRIPTION
 ...
 it does NOT require the Frontier VS Code extension or the core install.ps1.
...
.PARAMETER Source
 Path to the Frontier repository root (where .github/agents/ lives).
...
.EXAMPLE
 ...
 pwsh packs/frontier-copilot-cli/install.ps1

 # Install into a specific workspace
 pwsh packs/frontier-copilot-cli/install.ps1 -Target /path/to/my-project

 # Include CLI utilities
 pwsh packs/frontier-copilot-cli/install.ps1 -IncludeCli

 # Preview what would be installed
 pwsh packs/frontier-copilot-cli/install.ps1 -WhatIf
#>
...
if ($PSVersionTable.PSVersion -lt $MinimumPowerShellVersion) {
 Write-Host "[FAIL] Frontier Copilot CLI Plugin requires PowerShell 7.4+. Current version: $($PSVersionTable.PSVersion)" -ForegroundColor Red
 ...
 if ($IsWindows -or $env:OS -eq 'Windows_NT') {
  ...
  Write-Host "Then rerun with: pwsh packs/frontier-copilot-cli/install.ps1" -ForegroundColor DarkGray
 } else {
  Write-Host "Then rerun with: pwsh packs/frontier-copilot-cli/install.ps1" -ForegroundColor DarkGray
 }
 exit 1
...
function Initialize-WorkspaceCliState {
 ...
 $existingConfig = Read-JsonFile (Join-Path $stateRoot 'config.json')
 $existingVersion = Read-JsonFile (Join-Path $stateRoot 'version.json')
 $statusPath = Join-Path $stateRoot 'state' 'agent-status.json'
 ...
 if (-not $existingConfig -and $PSCmdlet.ShouldProcess((Join-Path $stateRoot 'config.json'), 'Write config')) {
  $config = [ordered]@{
   provider = 'local'
   integration = 'local'
   mode = 'local'
  ...
  $config | ConvertTo-Json -Depth 4 | Set-Content -Path (Join-Path $stateRoot 'config.json') -Encoding utf8
 }
 ...
 $version = [ordered]@{
  ...
  mode = 'local'
  integration = 'local'
  installedAt = if ($existingVersion -and $existingVersion.PSObject.Properties['installedAt'] -and $existingVersion.installedAt) { $existingVersion.installedAt } else { (Get-Date).ToUniversalTime().ToString('o') }
  updatedAt = (Get-Date).ToUniversalTime().ToString('o')
 }
 if ($PSCmdlet.ShouldProcess((Join-Path $stateRoot 'version.json'), 'Write version')) {
  $version | ConvertTo-Json -Depth 4 | Set-Content -Path (Join-Path $stateRoot 'version.json') -Encoding utf8
 }
}
...
if (-not $Source) {
 ...
 if (-not $Source) {
  Write-Err "Cannot auto-detect Frontier source. Use -Source to specify the Frontier repo root."
  exit 1
 }
}
...
Write-Host "| Frontier Copilot CLI Plugin v9.3.0        |" -ForegroundColor Cyan
...
Write-Host " Frontier Copilot CLI Plugin v9.3.0 installed" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
Write-Host ""
