...
<#
.SYNOPSIS
  Install Frontier into the per-user GitHub Copilot CLI customization directory.

.DESCRIPTION
  Path 2 installer. Copies Frontier FDEs, skills, instructions, and prompts into
  ...
  workspace). Use this when you want Frontier available everywhere you launch
  ...
  Also optionally registers the Frontier MCP server in ~/.copilot/mcp-config.json
  when -RegisterMcp is supplied (Path 3 hookup).

.PARAMETER Source
  Frontier repo root. Auto-detected from script location.
...
.PARAMETER RegisterMcp
  Add the Frontier MCP server entry to ~/.copilot/mcp-config.json.

.PARAMETER WhatIf
  Preview without writing.

.EXAMPLE
  pwsh packs/frontier-copilot-cli/install-user.ps1

.EXAMPLE
  pwsh packs/frontier-copilot-cli/install-user.ps1 -RegisterMcp -IncludeTemplates -Force
#>
...
function Resolve-SourceRoot {
  ...
  throw 'Cannot auto-detect Frontier source. Use -Source.'
}
...
function Register-FrontierMcpServer {
  [CmdletBinding(SupportsShouldProcess)]
  ...
  if (Test-Path $McpConfigPath) {
    ...
    $json = ([ordered]@{ mcpServers = [ordered]@{ frontier = $entry } } | ConvertTo-Json -Depth 8)
  }
  ...
  Write-OK "Registered Frontier MCP server in $McpConfigPath"
}
...
Write-Host '| Frontier -> Copilot CLI (user-level install)|' -ForegroundColor Cyan
Write-Host '=============================================' -ForegroundColor Cyan
...
foreach ($legalFile in @('LICENSE', 'NOTICE')) {
  $r = Copy-OneFile -Src (Join-Path $Source $legalFile) -Dest (Join-Path $copilotDir "frontier-legal/$legalFile") -Overwrite
  $totalCopied += $r.Copied; $totalSkipped += $r.Skipped
}
...
if ($PSCmdlet.ShouldProcess((Join-Path $copilotDir '.frontier-version.json'), 'Write version stamp')) {
  $stamp | ConvertTo-Json | Set-Content -Path (Join-Path $copilotDir '.frontier-version.json') -Encoding utf8
  Write-OK 'Wrote ~/.copilot/.frontier-version.json'
}

if ($RegisterMcp) {
  Register-FrontierMcpServer -McpConfigPath (Join-Path $copilotDir 'mcp-config.json') -SourceRoot $Source
}
...
Write-Host '  2. Frontier FDEs/skills/instructions are now discovered globally.'
if (-not $RegisterMcp) {
  Write-Host '  3. To also expose Frontier CLI as MCP tools, rerun with -RegisterMcp.'
}
Write-Host ''
