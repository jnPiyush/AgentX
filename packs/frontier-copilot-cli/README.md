# Frontier Copilot CLI Pack

> **Standalone distribution** for GitHub Copilot CLI. Separate from the VS Code extension and the core Frontier installation.
...
Frontier supports two Copilot CLI installation paths.

### Option 1 -- Native Copilot CLI plugin (recommended)

The repository root ships a `plugin.json`, so Copilot CLI can install Frontier's
...
Use this when you want Frontier committed to (or vendored in) a specific
workspace.
...
| Interactive chat | Chat participant (`@frontier`) | Not available -- use `copilot` sessions |
...
- User-level installs place Frontier legal files under
