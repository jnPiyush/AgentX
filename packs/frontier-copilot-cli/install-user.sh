#!/usr/bin/env bash
# Install Frontier into ~/.copilot/ for global GitHub Copilot CLI discovery.
...
#
# Optionally registers Frontier MCP server in ~/.copilot/mcp-config.json (--mcp).
...
Usage: install-user.sh [options]
  ...
  -s, --source PATH    Frontier repo root (auto-detected if omitted)
  ...
  -m, --mcp            Register Frontier MCP server in ~/.copilot/mcp-config.json
...
resolve_source() {
  ...
  log_err 'Cannot auto-detect Frontier source. Use --source.'
  exit 1
}
...
register_mcp() {
  ...
  if [ $DRY_RUN -eq 1 ]; then
    log_info "[DRY] would register Frontier MCP server in $cfg"
    return
  fi
  mkdir -p "$(dirname "$cfg")"
  FRONTIER_REPO="$repo" FRONTIER_CFG="$cfg" node - <<'NODE_EOF'
const fs = require('fs');
const path = require('path');
const cfgPath = process.env.FRONTIER_CFG;
const repo = process.env.FRONTIER_REPO;
let cfg = { mcpServers: {} };
...
console.log('[OK] Registered Frontier MCP server in ' + cfgPath);
NODE_EOF
}
...
printf '\n'
printf '=============================================\n'
printf '| Frontier -> Copilot CLI (user-level install)|\n'
printf '=============================================\n'
...
for legal_file in LICENSE NOTICE; do
  legal_target="$COPILOT_DIR/frontier-legal/$legal_file"
  if [ $DRY_RUN -eq 1 ]; then
...
JSON
  log_ok 'Wrote ~/.copilot/.frontier-version.json'
fi
...
printf '  2. Frontier FDEs/skills/instructions are now discovered globally.\n'
if [ $REGISTER_MCP -ne 1 ]; then
  printf '  3. To also expose Frontier CLI as MCP tools, rerun with --mcp.\n'
fi
printf '\n'
