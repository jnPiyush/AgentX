...
<#
.SYNOPSIS
 Install AgentX v8.4.43 - Download, copy, configure.
...
Write-Host "| AgentX v8.4.43 - AI Agent Orchestration |" -ForegroundColor Cyan
Write-Host "+===================================================+" -ForegroundColor Cyan
Write-Host ""
...
if ($previousVersion -and $previousVersion -ne "8.4.43") {
 $majorVersion = 0
 try { $majorVersion = [int]($previousVersion -split '\.')[0] } catch { Write-Verbose "Could not parse major version from '$previousVersion'." }

 if ($majorVersion -lt 8) {
    Write-Host "[!] Detected AgentX v$previousVersion - upgrading to v8.4.43..." -ForegroundColor Yellow
...
Write-OK "Version 8.4.43 recorded"
...
Write-Host " AgentX v8.4.43 installed! [$displayMode]" -ForegroundColor Green
Write-Host "===================================================" -ForegroundColor Green
Write-Host ""
