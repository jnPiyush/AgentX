...
<#
.SYNOPSIS
 Install AgentX v8.4.50 - Download, copy, configure.
...
Write-Host "| AgentX v8.4.50 - AI Agent Orchestration |" -ForegroundColor Cyan
Write-Host "+===================================================+" -ForegroundColor Cyan
Write-Host ""
...
if ($previousVersion -and $previousVersion -ne "8.4.50") {
 $majorVersion = 0
 try { $majorVersion = [int]($previousVersion -split '\.')[0] } catch { Write-Verbose "Could not parse major version from '$previousVersion'." }

 if ($majorVersion -lt 8) {
    Write-Host "[!] Detected AgentX v$previousVersion - upgrading to v8.4.50..." -ForegroundColor Yellow
...
Write-OK "Version 8.4.50 recorded"
...
Write-Host " AgentX v8.4.50 installed! [$displayMode]" -ForegroundColor Green
Write-Host "===================================================" -ForegroundColor Green
Write-Host ""
