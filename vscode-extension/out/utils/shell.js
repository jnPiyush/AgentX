...
function resolveWindowsShell() {
    ...
    try {
        (0, child_process_1.execSync)('pwsh -NoProfile -Command "exit 0"', { stdio: 'ignore', timeout: 5_000 });
        _resolvedPwsh = 'pwsh';
        return _resolvedPwsh;
    }
    ...
    if (process.platform === 'win32') {
        try {
            (0, child_process_1.execSync)('powershell.exe -NoProfile -Command "exit 0"', { stdio: 'ignore', timeout: 5_000 });
...
function execShell(command, cwd, shell = 'pwsh') {
    return new Promise((resolve, reject) => {
        ...
        if (shell === 'bash') {
            ...
            if (!resolved) {
                reject(new Error('PowerShell is not installed. Install PowerShell 7+ (pwsh) from '
