 ...
 */
function tryExec(command, timeoutMs = 10_000) {
    return new Promise((resolve) => {
        const shell = process.platform === 'win32' ? 'powershell.exe' : '/bin/bash';
        (0, child_process_1.exec)(command, { shell, timeout: timeoutMs }, (error, stdout) => {
            if (error) {
                resolve('');
                return;
            }
 ...
 */
async function checkAllDependencies(mode = 'local') {
    ...
    const checks = await Promise.all([
        ...
        checkGit(),
        checkGitHubCli(),
        checkVSCodeExtension('github.copilot', 'GitHub Copilot', 'required', 'Required for AI-powered agent interactions.'),
        checkVSCodeExtension('github.copilot-chat', 'GitHub Copilot Chat', 'required', 'Required for @agentx chat participant and agent routing.'),
    ]);
