...
export function generateAgentContent(details: AgentDetails): string {
  const constraintList = details.constraints
    .map((c) => `  - "${c}"`)
    .join('\n');
...
- Follow the Frontier quality loop and its runtime-computed risk minimum
