...
describe('FrontierContext', () => {
  ...
  describe('workspaceRoot', () => {
    ...
    it('should detect Frontier root at workspace folder level', () => {
    ...
    it('should fall back to first workspace folder if no Frontier root found', () => {
    ...
    it('should honor explicit frontier.rootPath setting', () => {
      ...
      createFrontierRoot(explicit);

      const other = path.join(tmpBase, 'other');
      fs.mkdirSync(other, { recursive: true });
      createFrontierRoot(other);

      __setWorkspaceFolders([{ path: other }]);
      __setConfig('frontier.rootPath', explicit);
    ...
    it('should ignore invalid explicit rootPath and fall back', () => {
      ...
      __setConfig('frontier.rootPath', path.join(tmpBase, 'does-not-exist-' + Date.now()));
  ...
  describe('checkInitialized', () => {
    it('should return true when Frontier root exists', async () => {
  ...
  describe('getCliCommand', () => {
    it('should return powershell path on Windows with auto shell', () => {
      ...
      createFrontierRoot(root);
      __setWorkspaceFolders([{ path: root }]);
      __setConfig('frontier.shell', 'auto');
      ...
      if (process.platform === 'win32') {
        ...
        assert.ok(cli.endsWith('frontier.sh'), 'should use SH on non-Windows');
      }
    });
    ...
    it('should return bash path when shell is forced to bash', () => {
      ...
      createFrontierRoot(root);
      __setWorkspaceFolders([{ path: root }]);
      __setConfig('frontier.shell', 'bash');
