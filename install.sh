#!/bin/bash
# AgentX v8.7.1 Installer - Download, copy, configure.
#
...
# curl -fsSL https://raw.githubusercontent.com/jnPiyush/AgentX/v8.7.1/install.sh | bash
#
...
echo ""
echo -e "${C}+===================================================+${N}"
echo -e "${C}| AgentX v8.7.1 - AI Agent Orchestration |${N}"
echo -e "${C}+===================================================+${N}"
echo ""
...
if [ -n "$PREVIOUS_VERSION" ] && [ "$PREVIOUS_VERSION" != "8.7.1" ]; then
 MAJOR_VERSION=$(echo "$PREVIOUS_VERSION" | cut -d. -f1)

 if [ "$MAJOR_VERSION" -lt 8 ] 2>/dev/null; then
  echo -e "${Y}[!] Detected AgentX v$PREVIOUS_VERSION - upgrading to v8.7.1...${N}"
...
echo "{ \"version\": \"8.7.1\", \"mode\": \"$MODE\", \"installedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"updatedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\" }" > "$VERSION_FILE"
ok "Version 8.7.1 recorded"
...
echo ""
echo -e "${G}===================================================${N}"
echo -e "${G} AgentX v8.7.1 installed! [$DISPLAY_MODE]${N}"
echo -e "${G}===================================================${N}"
echo ""
