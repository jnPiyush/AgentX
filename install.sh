#!/bin/bash
# AgentX v9.3.0 Installer - Download, copy, configure.
#
...
echo ""
echo -e "${C}+===================================================+${N}"
echo -e "${C}| AgentX v9.3.0 - AI Agent Orchestration |${N}"
echo -e "${C}+===================================================+${N}"
echo ""
...
if [ -n "$PREVIOUS_VERSION" ] && [ "$PREVIOUS_VERSION" != "9.3.0" ] && [ "$FORCE" != "true" ]; then
...
if [ -n "$PREVIOUS_VERSION" ] && [ "$PREVIOUS_VERSION" != "9.3.0" ]; then
 ...
 if [ "$MAJOR_VERSION" -lt 9 ] 2>/dev/null; then
  echo -e "${Y}[!] Detected AgentX v$PREVIOUS_VERSION - upgrading to v9.3.0...${N}"
...
echo ""
echo -e "${G}===================================================${N}"
echo -e "${G} AgentX v9.3.0 installed! [$DISPLAY_MODE]${N}"
echo -e "${G}===================================================${N}"
echo ""
