#!/bin/bash
# AgentX v8.4.61 Installer - Download, copy, configure.
#
...
echo ""
echo -e "${C}+===================================================+${N}"
echo -e "${C}| AgentX v8.4.61 - AI Agent Orchestration |${N}"
echo -e "${C}+===================================================+${N}"
echo ""
...
if [ -n "$PREVIOUS_VERSION" ] && [ "$PREVIOUS_VERSION" != "8.4.61" ]; then
 MAJOR_VERSION=$(echo "$PREVIOUS_VERSION" | cut -d. -f1)

 if [ "$MAJOR_VERSION" -lt 8 ] 2>/dev/null; then
  echo -e "${Y}[!] Detected AgentX v$PREVIOUS_VERSION - upgrading to v8.4.61...${N}"
...
echo "{ \"version\": \"8.4.61\", \"mode\": \"$MODE\", \"installedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"updatedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\" }" > "$VERSION_FILE"
ok "Version 8.4.61 recorded"
...
echo ""
echo -e "${G}===================================================${N}"
echo -e "${G} AgentX v8.4.61 installed! [$DISPLAY_MODE]${N}"
echo -e "${G}===================================================${N}"
echo ""
