#!/bin/bash
# AgentX v9.0.0 Installer - Download, copy, configure.
#
...
# curl -fsSL https://raw.githubusercontent.com/jnPiyush/AgentX/v9.0.0/install.sh | bash
#
...
echo ""
echo -e "${C}+===================================================+${N}"
echo -e "${C}| AgentX v9.0.0 - AI Agent Orchestration |${N}"
echo -e "${C}+===================================================+${N}"
echo ""
...
if [ -n "$PREVIOUS_VERSION" ] && [ "$PREVIOUS_VERSION" != "9.0.0" ]; then
 MAJOR_VERSION=$(echo "$PREVIOUS_VERSION" | cut -d. -f1)

 if [ "$MAJOR_VERSION" -lt 9 ] 2>/dev/null; then
  echo -e "${Y}[!] Detected AgentX v$PREVIOUS_VERSION - upgrading to v9.0.0...${N}"
...
echo "{ \"version\": \"9.0.0\", \"mode\": \"$MODE\", \"installedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"updatedAt\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\" }" > "$VERSION_FILE"
ok "Version 9.0.0 recorded"
...
echo ""
echo -e "${G}===================================================${N}"
echo -e "${G} AgentX v9.0.0 installed! [$DISPLAY_MODE]${N}"
echo -e "${G}===================================================${N}"
echo ""
